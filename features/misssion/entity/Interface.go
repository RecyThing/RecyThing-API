...
type TrashCategoryRepositoryInterface interface {
	...
	FindAll(page, limit int, trashType string) ([]TrashCategoryCore, pagination.PageInfo, int, error)
	GetCount(trashType string) (int, error)
}
...
type TrashCategoryServiceInterface interface {
	...
	GetAllCategory(page, limit, TrashType string) ([]TrashCategoryCore, pagination.PageInfo, int, error)
	GetById(idTrash string) (TrashCategoryCore, error)
}
