...
type RecybotRepositoryInterface interface {
	...
	FindAll(page, limit int, category string) ([]RecybotCore, pagination.PageInfo, error)
...
type RecybotServiceInterface interface {
	...
	FindAllData(page, category, limit string) ([]RecybotCore, pagination.PageInfo, error)
	GetById(idData string) (RecybotCore, error)
